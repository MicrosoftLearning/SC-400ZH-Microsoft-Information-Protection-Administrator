...
17. 勾选 **“限制对 Microsoft 365 位置中内容的访问或对这些内容进行加密”** 前的复选框，然后选择 **“阻止所有人”。只有内容所有者、最后的修改者和网站管理员可以继续访问**。

18. 在 **“创建规则”** 页面上的 **“用户通知”** 部分，选择开关将其切换至 **“开启”** 位置。

19. 在 **“创建规则”** 页面上的 **“用户覆盖”** 部分，选择开关将其切换至 **“开启”** 位置，并选择 **“需要提供业务正当理由才能进行覆盖”** 选项。

20. 在 **“事件报告”** 部分的 **“在管理警报和报告中使用此严重性”** 下拉菜单中，选择 **“低”**。

21. 在 **“事件报告”** 部分，选择 **“当出现与规则匹配的情况时，向管理员发送警报”** 开关将其切换至 **“开启”** 位置，并查看选项。默认设置将通知创建策略的用户。

22. 选择 **“保存”**，然后选择 **“下一步”**。

23. 在 **“测试或启用策略”** 页面上，选择 **“我想先对其进行测试”**，然后选择 **“在测试模式下显示策略提示”**。
...
5. 在 PowerShell 中输入以下命令，以将 DLP 规则添加到在步骤 4 中创建的 DLP 策略：
...
### 任务 6 - 在 Microsoft Cloud App Security (MCAS) 中启用文件监视

你需要使用 Microsoft Cloud App Security 中的文件策略来保护 OneDrive for Business 和 SharePoint Online 位置中的文件。在创建文件策略之前，需要启用文件监视，从而让 MCAS 扫描组织中的文件。
...
5. 选中 **“启用文件监视”** 复选框，然后选择 **“保存”**。

你已成功在 MCAS 中启用了文件监视，现在可以使用文件策略扫描文件，以查看是否存在敏感内容。

### 任务 7 - 为 MCAS 创建文件策略

在本任务中，你需要在 Microsoft Cloud App Security 中创建一个文件策略，用于扫描 OneDrive for Business 和 SharePoint Online 中的文件，并在共享文件时自动隔离包含信用卡信息的文件。
...
6. 将 **“策略严重性”** 保持为 **“低”**，并确保将 **“分类”** 设置为 **“DLP”**。这些应该是文件策略的默认设置。

7. 在 **“为该策略将要处理的文件创建筛选器”** 区域，展开下拉菜单 **“公开(Internet)、外部、公开”** 并添加 **“内部”**。
...
你现在已经创建了一个文件策略，该策略将持续扫描保存在 OneDrive for Business 和 SharePoint 中的文件，以查看是否存在信用卡信息，并在组织内部共享这些文件时隔离它们。
